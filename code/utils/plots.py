...
def plot(model, indices, model_outputs ,pose, rgb_gt, path, epoch, img_res, plot_nimgs, max_depth, resolution):
    ...
    depth = jt.ones(batch_size * num_samples).cuda().float() * max_depth
    ...
    for i, p, m in zip(indices, points, network_object_mask):
        p = p[m]
        sampling_idx = jt.randperm(p.shape[0])[:2048]
        p = p[sampling_idx, :]
...
def get_surface_trace(path, epoch, sdf, resolution=100, return_mesh=False):
    ...
    z = []
    for i, pnts in enumerate(jt.split(points, 100000, dim=0)):
...
def get_surface_high_res_mesh(sdf, resolution=100):
    ...
    recon_pc = jt.from_numpy(recon_pc).float().cuda()

    # Center and align the recon pc
    s_mean = recon_pc.mean(dim=0)
    s_cov = recon_pc - s_mean
    s_cov = jt.mm(s_cov.transpose(0, 1), s_cov)
    vecs = jt.eig(s_cov, True)[1].transpose(0, 1)
    if jt.det(vecs) < 0:
        vecs = jt.mm(jt.tensor([[1, 0, 0], [0, 0, 1], [0, 1, 0]]).cuda().float(), vecs)
    helper = jt.bmm(vecs.unsqueeze(0).repeat(recon_pc.shape[0], 1, 1),
    ...
    g = []
    for i, pnts in enumerate(jt.split(grid_points, 100000, dim=0)):
        g.append(jt.bmm(vecs.unsqueeze(0).repeat(pnts.shape[0], 1, 1).transpose(1, 2),
                           pnts.unsqueeze(-1)).squeeze() + s_mean)
    grid_points = jt.cat(g, dim=0)
    ...
    if (not (np.min(z) > 0 or np.max(z) < 0)):
        ...
        verts = jt.from_numpy(verts).cuda().float()
        verts = jt.bmm(vecs.unsqueeze(0).repeat(verts.shape[0], 1, 1).transpose(1, 2),
...
def get_grid_uniform(resolution):
    ...
    grid_points = jt.tensor(np.vstack([xx.ravel(), yy.ravel(), zz.ravel()]).T, dtype=jt.float)
...
def get_grid(points, resolution):
    eps = 0.2
    input_min = jt.min(points, dim=0)[0].squeeze().numpy()
    input_max = jt.max(points, dim=0)[0].squeeze().numpy()
    ...
    xx, yy, zz = np.meshgrid(x, y, z)
    grid_points = jt.tensor(np.vstack([xx.ravel(), yy.ravel(), zz.ravel()]).T, dtype=jt.float).cuda()
...
def plot_images(rgb_points, ground_true, path, epoch, plot_nrow, img_res):
    ...
    output_vs_gt = jt.cat((rgb_points, ground_true), dim=0)
