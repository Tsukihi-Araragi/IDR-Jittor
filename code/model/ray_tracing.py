...
class RayTracing(Module):
    ...
    def execute(self, 
        ...
        sampler_net_obj_mask = jt.zeros_like(sampler_mask).bool().cuda()
        if sampler_mask.sum() > 0:
            sampler_min_max = jt.zeros((batch_size, num_pixels, 2)).cuda()
        ...
        if mask_left_out.sum() > 0:  # project the origin to the not intersect points on the sphere
            ...
            acc_start_dis[mask_left_out] = -jt.bmm(rays_left_out.view(-1, 1, 3), cam_left_out.view(-1, 3, 1)).squeeze()
    ...
    def sphere_tracing(self, batch_size, num_pixels, sdf, cam_loc, ray_directions, mask_intersect, sphere_intersections):
        ...
        curr_start_points = jt.zeros(batch_size * num_pixels, 3).cuda().float()
        curr_start_points[unfinished_mask_start] = sphere_intersections_points[:,:,0,:].reshape(-1,3)[unfinished_mask_start]
        acc_start_dis = jt.zeros(batch_size * num_pixels).cuda().float()
        ...
        curr_end_points = jt.zeros(batch_size * num_pixels, 3).cuda().float()
        curr_end_points[unfinished_mask_end] = sphere_intersections_points[:,:,1,:].reshape(-1,3)[unfinished_mask_end]
        acc_end_dis = jt.zeros(batch_size * num_pixels).cuda().float()
        ...
        while True:
            ...
            curr_sdf_end = jt.zeros_like(acc_end_dis).cuda()
    ...
    def ray_sampler(self, sdf, cam_loc, object_mask, ray_directions, sampler_min_max, sampler_mask):
        ...
        sampler_pts = jt.zeros(n_total_pxl, 3).cuda().float()
        sampler_dists = jt.zeros(n_total_pxl).cuda().float()

        intervals_dist = jt.linspace(0, 1, steps=self.n_steps).cuda().view(1, 1, -1)
        ...
        sdf_val = jt.cat(sdf_val_all).reshape(-1, self.n_steps)

        tmp = jt.sign(sdf_val) * jt.arange(self.n_steps, 0, -1).cuda().float().reshape((1, self.n_steps))  # Force argmin to return the first min value
        sampler_pts_ind = jt.argmin(tmp, -1)
        ...
        if n_p_out > 0:
            out_pts_idx = jt.argmin(sdf_val[p_out_mask, :], -1)
            sampler_pts[mask_intersect_idx[p_out_mask]] = points[p_out_mask, :, :][jt.arange(n_p_out), out_pts_idx, :]
            sampler_dists[mask_intersect_idx[p_out_mask]] = pts_intervals[p_out_mask, :][jt.arange(n_p_out), out_pts_idx]
        ...
        if n_secant_pts > 0:
            ...
            z_low = pts_intervals[secant_pts][jt.arange(n_secant_pts), sampler_pts_ind[secant_pts] - 1]
            sdf_low = sdf_val[secant_pts][jt.arange(n_secant_pts), sampler_pts_ind[secant_pts] - 1]
    ...
    def minimal_sdf_points(self, num_pixels, sdf, cam_loc, ray_directions, mask, min_dis, max_dis):
        ...
        n = self.n_steps
        # steps = jt.linspace(0.0, 1.0,n).cuda()
        steps = jt.empty(n).uniform_(0.0, 1.0).cuda()
        ...
        mask_sdf_all = jt.cat(mask_sdf_all).reshape(-1, n)
        min_vals, min_idx = mask_sdf_all.min(-1)
        min_mask_points = mask_points_all.reshape(-1, n, 3)[jt.arange(0, n_mask_points), min_idx]
        min_mask_dist = steps.reshape(-1, n)[jt.arange(0, n_mask_points), min_idx]
